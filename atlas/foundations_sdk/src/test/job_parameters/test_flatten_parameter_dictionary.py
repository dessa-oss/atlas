...
class TestFlattenParameterDictionary(Spec):
    ...
    def test_multiple_keys_with_at_most_singly_nested_values(self):
        ...
        list_of_keys = map(lambda list_index: f'non_empty_list_key_{list_index}', range(self.random_length))
        list_output = {key: value for key, value in zip(list_of_keys, self.random_literal_list_value)}

        expected_output.update(list_output)
        expected_dict_output = {f'non_empty_dictionary_key_{nested_key}': nested_value for nested_key, nested_value in self.random_literal_dict_value.items()}
