...
class TestAtlasParser(Spec):
    ...
    find_environment_mock = let_patch_mock('foundations_cli.environment_fetcher.EnvironmentFetcher.find_environment')
    ...
    def test_sub_parser_setup_parser_on_cli_instantiation(self):
        mock_add_parser = self.patch('foundations_cli.sub_parsers.atlas.atlas_parser.AtlasParser.add_sub_parser')
    ...
    def test_submit_forwards_default_arguments_to_command_line_job_submission(self):
        self.patch('foundations_cli.job_submission.submit_job.submit', MockCommandLineJobDeployer)
