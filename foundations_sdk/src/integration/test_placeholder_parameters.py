...
class TestPlaceHolderParameters(unittest.TestCase):
    ...
    def test_can_replace_input(self):
        def method(value):
            return value
        stage = foundations_context.pipeline().stage(method, Hyperparameter("value"))
        self.assertEqual(9, stage.run_same_process(value=9))

    def test_can_replace_input_with_different_name(self):
        def method(value):
            return value
        stage = foundations_context.pipeline().stage(method, Hyperparameter("special_value"))
        self.assertEqual(9, stage.run_same_process(special_value=9))

    def test_can_replace_keyword_parameter(self):
        def method(value):
            return value
        stage = foundations_context.pipeline().stage(method, value=Hyperparameter())
        self.assertEqual(9, stage.run_same_process(value=9))

    def test_can_replace_multiple_parameters(self):
        def method(value, value2):
            return value + value2
        stage = foundations_context.pipeline().stage(method, value=Hyperparameter(), value2=Hyperparameter())
    ...
    def test_can_replace_multiple_parameters_same_placeholder(self):
        ...
        stage = foundations_context.pipeline().stage(method, param, param)
    ...
    def test_can_mix_with_normal_parameters(self):
        def method(value, value2):
            return value + value2
        param = Hyperparameter("input")
        stage = foundations_context.pipeline().stage(method, param, 3)
    ...
    def test_supports_deep_placeholders(self):
        ...
        stage = foundations_context.pipeline().stage(make_value, Hyperparameter("input"))
        stage2 = foundations_context.pipeline().stage(scale, stage, Hyperparameter("scale"))
