...
class TestStageConnectorWrapper(unittest.TestCase):
    ...
    def test_stage_generates_uuid_with_arugments(self):
        ...
        stage = builder.build(self._make_stage)
        self.assertEqual('f038d3426aae8101d6687a4067925c26a125d46d', stage.uuid())

    def test_stage_generates_uuid_with_multiple_arugments(self):
        builder = StageConnectorWrapperBuilder(self._graph, self._pipeline_context)
        builder.stage('start', self._method_two, (3, 5), {})
        stage = builder.build(self._make_stage)
        self.assertEqual('caf0a4d242f7ddc17bf619380241ce8d6e7b4d18', stage.uuid())

    def test_stage_generates_uuid_with_keyword_arguments(self):
        builder = StageConnectorWrapperBuilder(self._graph, self._pipeline_context)
        builder.stage('start', self._method_two, (), {'hello': 'world'})
        stage = builder.build(self._make_stage)
        self.assertEqual('b81e43e394d681d282ad6f3b7bd6c81e8159f8a2', stage.uuid())

    def test_stage_generates_uuid_with_keyword_arguments_different_values(self):
        builder = StageConnectorWrapperBuilder(self._graph, self._pipeline_context)
        builder.stage('start', self._method_two, (), {'spinach': 'potato', 'rabbit': 'monkey'})
        stage = builder.build(self._make_stage)
        self.assertEqual('be0644ee25ea3cc012fa640bd1b7f4a7ca51550a', stage.uuid())
