...
class TestPipelineContextListing(unittest.TestCase):
    ...
    @patch('foundations.job_persister.JobPersister.load_archiver_fetch')
    @patch('foundations.models.pipeline_context_with_archive.PipelineContextWithArchive', MockPipelineContext)
    ...
    def test_yield_a_single_context_with_multiple_archivers(self, mock_fetch):
        ...
        results_with_contexts = [(job_id, context.archive) for job_id, context in results]

        self.assertEqual([('my job', archiver), ('my different job', archiver2)], results_with_contexts)
