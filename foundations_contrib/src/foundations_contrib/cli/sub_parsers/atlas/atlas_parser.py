...
class AtlasParser(object):
    ...
    def _initialize_submit_parser(self):
        ...
        deploy_parser = self._cli.add_sub_parser('submit', help='Deploys a Foundations project to the specified environment')
    ...
    def _initialize_retrieve_parser(self):
        retrieve_parser = self._cli.add_sub_parser('get', help='Get file types from execution environments')
    ...
    def _initialize_clear_queue(self):
        clear_queue_parser = self._cli.add_sub_parser('clear-queue', help='Clears all scheduled jobs from the queue')
    ...
    def _initialize_delete_parser(self):
        delete_parser = self._cli.add_sub_parser('delete', help='Delete items from execution environment')
    ...
    def _retrieve_artifacts(self):
        ...
        if job_status is None:
            self._cli._fail_with_message('Error: Job `{}` does not exist for environment `{}`'.format(job_id, env_name))
        else:
    ...
    def _retrieve_logs(self):
        ...
        if job_status is None:
            ...
            self._cli._fail_with_message('Error: Job `{}` is queued and has not produced any logs'.format(job_id))
        else:
    ...
    def _stop(self):
        ...
        try:
            ...
            if job_status is None:
                ...
                self._cli._fail_with_message('Error: Job `{}` is queued and cannot be stopped'.format(job_id))
            elif job_status == 'completed':
                self._cli._fail_with_message('Error: Job `{}` is completed and cannot be stopped'.format(job_id))
            else:
    ...
    def _delete_job(self):
        ...
        if job_status is None:
            ...
            self._cli._fail_with_message('Error: Job `{}` has status `{}` and cannot be deleted'.format(job_id, job_status))
        else:
