...
class TestStageLogger(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self._logger = StageLogger(
            self._pipeline_context, self._stage, self._stage_config, self._stage_context)
    ...
    def test_log_metric_stores_current_time(self):
        with freeze_time():
            ...
            self.assertDictContainsSubset(
                {'timestamp': time()}, self._stage_context.stage_log[0])

    def test_log_metric_stores_metric_multiple_metrics(self):
        self._logger.log_metric('rocauc', 0.77)
        self._logger.log_metric('loss', 0.553)
        self.assertEqual({'rocauc': 0.77, 'loss': 0.553},
                         self._stage_log_to_dict())
    ...
    def test_log_metric_supports_larger_lists(self):
        ...
        self.assertEqual(
            {'accuracy': [0.73, 0.75, 0.74]}, self._stage_log_to_dict())

    def test_pipeline_context(self):
        self.assertEqual(self._pipeline_context,
                         self._logger.pipeline_context())
