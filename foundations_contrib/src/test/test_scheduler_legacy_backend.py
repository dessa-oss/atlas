...
class TestSchedulerLegacyBackend(unittest.TestCase):
    ...
    def test_get_queued_jobs_no_jobs(self):
        ...
        backend = LegacyBackend(
            MockClock(), MockBucketStatScanner, 'jobs', 'archives', 'results')
    ...
    def test_get_queued_jobs_one_job(self):
        ...
        backend = LegacyBackend(
            MockClock(55), MockBucketStatScanner, 'jobs', 'archives', 'results')

        queued_jobs = list(backend.get_paginated(None, None, status="QUEUED"))
        self.assertEqual(queued_jobs, [JobInformation(
            'this_file', 22, 33, "QUEUED", "me")])
    ...
    def test_get_queued_jobs_returns_non_list_iterable(self):
        ...
        self.assertTrue(hasattr(queued_jobs, "__iter__")
                        and not isinstance(queued_jobs, list))
    ...
    def test_get_queued_jobs_one_job_different_job_different_time(self):
        ...
        backend = LegacyBackend(
            MockClock(100), MockBucketStatScanner, 'jobs', 'archives', 'results')

        queued_jobs = list(backend.get_paginated(None, None, status="QUEUED"))
        self.assertEqual(queued_jobs, [JobInformation(
            'that_file', 23, 77, "QUEUED", "you")])
    ...
    def test_get_queued_jobs_two_jobs_different_bucket(self):
        ...
        self._add_to_bucket(
            'jobs2', {'owner': 'me', 'last_modified': 22, 'filename': 'this_file.tgz'})
        self._add_to_bucket(
            'jobs2', {'owner': 'you', 'last_modified': 23, 'filename': 'that_file.tgz'})
        backend = LegacyBackend(
            MockClock(100), MockBucketStatScanner, 'jobs2', 'archives', 'results')
    ...
    def test_get_running_jobs_no_jobs(self):
        ...
        running_jobs = list(backend.get_paginated(
            None, None, status="RUNNING"))
    ...
    def test_get_running_jobs_one_job(self):
        ...
        running_jobs = list(backend.get_paginated(
            None, None, status="RUNNING"))
        self.assertEqual(running_jobs, [JobInformation(
            'this_file', 22, 78, "RUNNING", "me")])
    ...
    def test_get_running_jobs_two_jobs_different_bucket(self):
        ...
        backend = LegacyBackend(
            MockClock(100), MockBucketStatScanner, 'jobs', 'archives2', 'results')
    ...
    def test_get_completed_jobs_five_jobs_three_running_two_done(self):
        ...
        self._add_to_bucket('archives', {'owner': 'they', 'last_modified': 22, 'filename': 'it_file.tgz'})
        self._add_to_bucket('archives', {'owner': 'them', 'last_modified': 24, 'filename': 'what_file.tgz'})
        self._add_to_bucket('archives', {
                            'owner': 'pippin', 'last_modified': 26, 'filename': 'where_file.tgz'})
        self._add_to_bucket('results', {'owner': 'they', 'last_modified': 30, 'filename': 'it_file.tgz'})
        self._add_to_bucket('results', {'owner': 'them', 'last_modified': 40, 'filename': 'what_file.tgz'})
        backend = LegacyBackend(
            MockClock(100), MockBucketStatScanner, 'jobs', 'archives', 'results')

        completed_jobs = list(backend.get_paginated(
            None, None, status="COMPLETED"))
