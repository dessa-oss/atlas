describe.skip('Test Nested Jobs', () => { // wait_for_deployment_to_complete doesn't seem to work properly with nested jobs from remote deployments; unskip this when the pipeline is properly built
  ...
  const command = `export FOUNDATIONS_HOME=\`pwd\`/cypress/fixtures/atlas_scheduler/.foundations && cd cypress/fixtures/atlas_scheduler/nested_jobs/ && foundations login http://${schedulerIP}:5558 -u test -p test && foundations submit scheduler ${projectName} main.py`;
