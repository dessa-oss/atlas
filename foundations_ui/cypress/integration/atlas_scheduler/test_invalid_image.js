  ...
  const states = [
    {
      ...
      command: 'export FOUNDATIONS_HOME=`pwd`/cypress/fixtures/atlas_scheduler/.foundations && cd cypress/fixtures/atlas_scheduler/invalid_image && foundations login http://localhost:5558 -u test -p test && foundations submit scheduler invalid_image_project main.py',
    },
    {
      ...
      jobStatus: 'completed',
      logs: '',
      command: 'export FOUNDATIONS_HOME=`pwd`/cypress/fixtures/atlas_scheduler/.foundations && cd cypress/fixtures/atlas_scheduler/invalid_image/invalid_image_project && foundations login http://localhost:5558 -u test -p test && python main.py',
    },
  ];
