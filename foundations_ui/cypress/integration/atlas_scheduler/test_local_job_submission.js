  ...
  const states = [
    {
      ...
      command: `export FOUNDATIONS_HOME=\`pwd\`/cypress/fixtures/atlas_scheduler/.foundations && ${loginCommand} && cd cypress/fixtures/atlas_scheduler/local_job_submission/ && python local_job_submission_project/main.py 0 "Complete_Task_1" dummy`
    },
    {
      testName: `Test Local Job Submission with CLI from inside job directory`,
      projectName: 'local_job_submission_project',
      command: `export FOUNDATIONS_HOME=\`pwd\`/cypress/fixtures/atlas_scheduler/.foundations && ${loginCommand} && cd cypress/fixtures/atlas_scheduler/local_job_submission/local_job_submission_project && python main.py 0 "Complete_Task_2" dummy`
    }
  ];
