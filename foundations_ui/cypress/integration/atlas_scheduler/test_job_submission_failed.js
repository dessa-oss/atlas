  ...
  const states = [
    {
      ...
      command: `export FOUNDATIONS_HOME=\`pwd\`/cypress/fixtures/atlas_scheduler/.foundations && cd cypress/fixtures/atlas_scheduler/job_submission/ && python -m foundations login http://${schedulerIP}:5558 -u test -p test && python -m foundations submit scheduler job_submission_project main.py 0 "Complete_Failed_Task_1"`,
    },
    {
      ...
      command: `export FOUNDATIONS_HOME=\`pwd\`/cypress/fixtures/atlas_scheduler/.foundations && cd cypress/fixtures/atlas_scheduler/job_submission/ && python -m foundations login http://${schedulerIP}:5558 -u test -p test && python -m foundations submit scheduler job_submission_project_with_foundations main.py 0 "Complete_Failed_Task_2"`,
    },
  ];
