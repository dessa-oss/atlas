...
import foundations_internal.safe_inspect as inspect
...
class TestSafeInspect(unittest.TestCase):
    def test_try_get_source_code_in_memory(self):
        ...
        with patch('uuid.uuid4', fake_uuid4(uuid)):
            self.assertEqual(inspect.getsource(self._my_function),
                             "<could not get source code ({})>".format(uuid))

    def test_try_get_source_code_builtin(self):
        uuid = "that_uuid"

        with patch('uuid.uuid4', fake_uuid4(uuid)):
            self.assertEqual(inspect.getsource(
                len), "<could not get source code ({})>".format(uuid))
